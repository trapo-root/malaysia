...
window.selectCallback = function (variant, selector, $element) {
  ...
  if (variant.compare_at_price > variant.price) {
    ...
    var savingText = "(SAVE "+ percentage +"%)";
    var newElement = $('<span class="savings">'+savingText+'</span>');
    console.log(percentage);
    $(".payment-terms-container").after(newElement);
    //$("span.savings").text(savingText);
  }else{
    $('span.savings').remove();
  }
   var variantTitle = variant.title;
  var productHandle = $(".product_form ").data('product-handle');
  /*if(productHandle == 'trapo-i-sight-t350-dashcam' || productHandle == 'trapo-i-sight-t550-dashcam'){
  if(variantTitle == 'Dashcam with OBD Hardwire / None' || variantTitle == 'Dashcam with OBD Hardwire / 64GB' || variantTitle == 'Dashcam with OBD Hardwire / 128GB' ){
    $('.product-text p').text('(Ships out in 30 working days)');
    setTimeout( function(){ 
    $('.add-cart-text').text('Pre Order');
    }  , 1000 );
  }else{
    $('.product-text p').text('(Ships out in 2 working days)');
  ...
  if(productHandle == 'car-hydrophobic-wiper-blade'){
  if(variantTitle.indexOf(str2) != -1){
    
    setTimeout( function(){ 
      $('.product-text p').text('(Ships out in 30 working days)');
    $('.add_to_cart .text').text('Pre-Order');
    }  , 1000 );
  }else{
    $('.product-text p').text('(Ships out in 2 working days)');
  ...
  var str4 = "24''";
  if(productHandle == 'car-hydrophobic-wiper-blade'){
  if(variantTitle.indexOf(str4) != -1){
  ...
  var str4 = '300ML';
  if(productHandle == 'trapo-stainoff'){
  if(variantTitle.indexOf(str4) != -1){
    $('.product-text p').text('(Ships out in 7 working days)');
    setTimeout( function(){ 
    $('.add_to_cart .text').text('Pre-Order');
    }  , 1000 );
  }else{
  ...
  if(productHandle == 'car-rainguard-repellant'){
  if(variantTitle.indexOf(str4) != -1){
    $('.product-text p').text('(Ships out in 5 working days)');
    setTimeout( function(){ 
    $('.add_to_cart .text').text('Pre-Order');
    }  , 1000 );
  }else{
  ...
  var productType = $(".product_form ").data('product-type');
  var wiperVar = 'Rear';
  if(productType == 'WIPER' && productHandle != 'trapo-hydrophobic-wiper-blade-rear'){
    if(variantTitle.indexOf(wiperVar) != -1){
    $('.product-text p').text('(Ships out in 9 working days)');
    setTimeout( function(){ 
    $('.add_to_cart .text').text('Pre-Order');
    }  , 1000 );
  }else{
    $('.product-text p').text('(Ship out next working day)');
    $('.add_to_cart .text').text('Add to Cart');
  }
  }

  var selectedVarCmp = variant.compare_at_price;
  
  if (selectedVarCmp !== null){
    var finalVarCmp = window.Shopify.formatMoney(selectedVarCmp, $('body').data('money-format'));
    $(".footer-p-price").find(".was-price").html(finalVarCmp);
  }else{
   $(".footer-p-price").find(".was-price").empty();
  }
  
  var selectedVarPrice = variant.price;
  var finalVarPrice = window.Shopify.formatMoney(selectedVarPrice, $('body').data('money-format'));

  $(".footer-p-price").find(".money").html(finalVarPrice);
...
window.productPage = {
  ...
  runOptionSelector($element) {
    ...
    $defaultSelector.each((_, options) => {
      ...
      const isCampaignEnabled = priceUIEl && priceUIEl.closest(['[data-display-campaign="true"]']);
...
window.header = {
  init() {
    ...
    var openMiniCart = function($cart_container) {
	  ...
	  $('body').addClass('blocked-scroll').addClass('minicart-open');
    };
	...
	$('.minicart-header-close-btn svg').on('click', () => {
      closeMiniCart();
    });
  },
  unload() {
