...
$(() => {
  ...
  function refreshCart(cart) {
    ...
    //cartItemsHTML += `<div class="minicart-box-out-bar"> <h3><span>${cart.item_count}</span> Products</h3> <div class="cart-slider-pointer" role="button"></div></div>`;
    ...
    if (cart.item_count === 0) {
	  ...
	  console.log(cart);
	  $('.minicart-box-out-bar h3 span').text(cart.item_count)
      ...
      $.each(cart.items, (index, item) => {
        ...
        cartItemsHTML += `<div class="mini-cart__item-content"><div class="mini-cart__item-title"><a href="${item.url}" class="cart_item-product-title">${item.product_title}</a>`;
		...
		if (item.variant_title != null) {
          cartItemsHTML += '<div class="mini-cart__item-properties">';
              ...
              cartItemsHTML += `${item.variant_title}`;
         
          cartItemsHTML += '</div>';
        }

        cartItemsHTML += '</div><div class="mini-cart__item-bottom"><div class="mini-cart__item-price">';
        ...
        $.ajax({
          ...
          success(data) {
            ...
            if (productData.compare_at_price && productData.compare_at_price > productData.price) {
              productHasSale = true;
              productCompareAtPrice = productData.compare_at_price * item.quantity;
              productFinalPrice = productData.price * item.quantity;
            } else {
        ...
        if (productHasSale === true) {
          ...
          // puts the slash through the old item price
		  const itemFinal_price = item.final_price * item.quantity;
		  const itemPrice_main = item.price * item.quantity;
          const itemPrice = `${window.Shopify.formatMoney(itemFinal_price, $('body').data('money-format'))} </span><span class="money was_price">${window.Shopify.formatMoney(itemPrice_main, $('body').data('money-format'))}</span>`;
          cartItemsHTML += `<span class="money sale">${itemPrice}</strong>`;
        } else {
          ...
          if (item.price > 0) {
            ...
            cartItemsHTML += `<span class="subtotal-text">${window.PXUTheme.translation.cart_free_text}</span></strong>`;
          }
        }
      ...
      cartActionHTML += `<span class="right"><span class="money">${window.Shopify.formatMoney(cart.total_price, $('body').data('money-format'))}</span></span> <span class="subtotal-text">${window.PXUTheme.translation.cart_subtotal_text}</span>`;
  ...
  $('body').on('click', '.cart-slider-pointer', function (e) {
    e.preventDefault();
...
$(".mobile_btn").on("click", function() {
    ...
    $('.main_menu.active.nav__item').toggle();
});
$(function($) {
    $(" .mobile_title_2").on("click", function() {
        $(this)
            .parent("  .has_dropdown")
            .children(" .sub_menu")
            .stop()
...
$(document).ready(function($) {
    $('.has_dropdown_2').find('.sub_menu_2_title').click(function() {
        var isActive = $(this).hasClass("active");
        $('.sub_menu_2_title').removeClass('active')
        if (!isActive) {
            $(this).toggleClass('active');
        }
        $(this).next().slideToggle('fast');
        $(".sub_menu_2").not($(this).next()).slideUp('fast');
    });
});
...
$(' .nav__link').click(function(e) {
    ...
    $('.nav__link.sub__close').html(currentTitle);
    $(this).siblings().addClass('is-active');
});
$("#mobileButton").click(function() {
    $(".nav__sub.mobile_view").removeClass("is-active");
});

$("a.sub_menu_2_title").hover(function() {
    var contentAsset = $(this).attr("data-product");
    $(".product-img").hide();
    $('[alt="' + contentAsset + '"]').show();
});

$(".main_menu .main_menu_list li .sub_menu_2 a").hover(function() {
    var contentAsset = $(this).attr("data-product");
    console.log(contentAsset);
    $(".product-img").hide();
    $('[alt="' + contentAsset + '"]').show();
});
$(".submenu_list li a").hover(function() {
    var contentAsset = $(this).attr("data-product");
    $(".product-img").hide();
    $('[alt="' + contentAsset + '"]').show();
})
...
$(document).ready(function($) {
    $('.footer_sub_col').find('.footer_title').click(function() {
        ...
        $(".footer_mb").not($(this).next()).slideUp('fast');

    });
});

/** End Header & Footer Script **/